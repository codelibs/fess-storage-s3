 ...
 *
 * <p>Registered as the LastaDi components {@code s3StorageClient} and {@code s3_compatStorageClient}
 * by fess_storage++.xml, which is how {@code storage.type=s3} and the S3_COMPAT auto-detection
 * result reach it. It lived in Fess itself until 15.9; it moved here with the AWS SDK for Java so
 * that the distribution does not carry the SDK for a backend most installations do not use.</p>
 */
